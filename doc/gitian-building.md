...
*Setup instructions for a Gitian build of AllCoinGuru Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AllCoinGuru
...
- [Building AllCoinGuru Core](#building-allcoinguru-core)
...
Clone the git repositories for AllCoinGuru Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/allcoinguruproject/allcoinguru
```
...
in the AllCoinGuru Core repository under 'Fetch and build inputs' to install sources which require
...
To build AllCoinGuru Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the AllCoinGuru Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/allcoinguru/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/allcoinguruproject/allcoinguru
...
For example:
```bash
URL=https://github.com/crowning-/allcoinguru.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit allcoinguru=${COMMIT} --url allcoinguru=${URL} ../allcoinguru/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit allcoinguru=${COMMIT} --url allcoinguru=${URL} ../allcoinguru/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit allcoinguru=${COMMIT} --url allcoinguru=${URL} ../allcoinguru/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the allcoinguru git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../allcoinguru/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/allcoinguru.git
SIGPATH=/some/root/path/allcoinguru-detached-sigs.git

./bin/gbuild --url allcoinguru=${BTCPATH},signature=${SIGPATH} ../allcoinguru/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/allcoinguru-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/allcoinguru-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/allcoinguru-osx-build.assert
```
...
[allcoinguru/gitian.sigs](https://github.com/allcoinguruproject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
