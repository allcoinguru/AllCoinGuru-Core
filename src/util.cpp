...
const char * const BITCOIN_CONF_FILENAME = "allcoinguru.conf";
const char * const BITCOIN_PID_FILENAME = "allcoingurud.pid";
 ...
 * by well-meaning allcoinguru adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "allcoinguru" is a composite category enabling all AllCoinGuru-related debug output
            if(ptrCategory->count(string("allcoinguru"))) {
...
#else
    const char* pszModule = "allcoinguru";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AllCoinGuruCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AllCoinGuruCore
    // Mac: ~/Library/Application Support/AllCoinGuruCore
    // Unix: ~/.allcoingurucore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AllCoinGuruCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AllCoinGuruCore";
#else
    // Unix
    return pathRet / ".allcoingurucore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty allcoinguru.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override allcoinguru.conf
